...
* Your FMS (or similar) server must support a <tt>getStreamLength</tt> call to get the length of the music stream.

There is now an <tt>onconnect</tt> callback which is called when the connection to the server is made which you can use if you wish.  Pass it like the other callbacks in your call to <tt>createSound</tt>.
