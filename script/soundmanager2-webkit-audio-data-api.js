...
function SoundManager(smURL, smID) {
  ...
  this.versionNumber = 'V2.97a.20120624+DEV';
  this.version = null;
  this.movieURL = null;
  this.altURL = null;
  ...
  function WebkitAudioWrapper(audio) {

    // EXPERIMENTAL: Webkit Audio API test
    ...
    // analyser.fft = 256; // match flash 9?
    ...
    function analyse() {
      ...
      var freqOut = [], timeOut = [], i, j;

      for (i=0, j=freqByteData.length; i<j; i++) {
        freqOut[i] = freqByteData[i*4]/256;
      }

      for (i=0, j=timeDomainData.length; i<j; i++) {
        timeOut[i] = timeDomainData[i*4]/256 * -1;
      }
      ...
      // console.log(freqByteData, timeDomainData);
    ...
    audio.addEventListener('canplay', connect, false);
...
*/
  ...
  SMSound = function(oOptions) {
    ...
    _add_html5_events = function() {
      ...
      if (!webkitAudioWrapper) {
        webkitAudioWrapper = new WebkitAudioWrapper(_t._a);
      }
...
var webkitFreqByteData = webkitAudioWrapper.analyse().freqByteData;
var webkitTimeDomainData = webkitAudioWrapper.analyse().timeByteData;

        // webkit audio API crap?
        if (webkitAudioWrapper) {

          _t.eqData = webkitFreqByteData;
          _t.eqData.left = webkitFreqByteData;
          _t.eqData.right = webkitFreqByteData;

          _t.waveformData = webkitTimeDomainData;
          _t.waveformData.left = webkitTimeDomainData;
          _t.waveformData.right = webkitTimeDomainData;
...
// HACK for webkit audio test shiz
_s.features.eqData = true;
_s.features.waveformData = true;
