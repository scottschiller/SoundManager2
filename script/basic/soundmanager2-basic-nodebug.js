...
function SoundManager(smURL, smID) {
  ...
  this.audioFormats = {
    ...
    // eg. if MP3 required, Flash fallback is used if HTML5 can't play it
    ...
    'mp4': {
      ...
      'required': false // use full soundmanager2.js + flash 9 if required, no MP4 in flash 8.
    },
    'ogg': {
  ...
  this.flashVersion = 8; // "SM2 basic" version = flash 8-only
  ...
  */
  ...
  this.createSound = function(oOptions) {
    ...
    */
    ...
    if (_html5OK(_tO)) {
      ...
      oSound = make();
      _s.o._createSound(_tO.id, 0, _tO.loops||1, false);
    }
  ...
  SMSound = function(oOptions) {
    ...
    this.load = function(oOptions) {
      ...
      if (_html5OK(_t._iO)) {
        ...
        try {
          ...
          _s.o._load(_t.sID, _t._iO.url, _t._iO.stream, _t._iO.autoPlay, (_t._iO.whileloading?1:0), _t._iO.loops||1, false);
        } catch(e) {
          //_wDS('smError', 2);
    ...
    this.play = function(oOptions, _updatePlayState) {
      ...
      if (_t.paused && _t.position && _t.position > 0) { // https://gist.github.com/37b17df75cc4d7a90bf6
        ...
        if (!_t.isHTML5) {
          _s.o._start(_t.sID, _t._iO.loops || 1, (_t.position / 1000));
        } else {
          _start_html5_timer();
    ...
    this._whileplaying = function(nPosition) {
      ...
      if (_t.playState === 1) {
        ...
        if (!_t.isHTML5 && !_t.position && _t.isBuffering) {
          _t._onbufferchange(0);
        }
  ...
  _strings = {
    ...
    needfl9: 'Note: Flash 9 required for MP4 formats, not in "basic" version; use full soundmanager2.js.',
  ...
  _setVersionInfo = function() {
    ...
    if (_s.useHTML5Audio && _s.audioFormats.mp4.required) {
      //_s._wD(_str('needfl9'));
    }
    _fV = _s.flashVersion; // short-hand for internal use
    _s.version = _s.versionNumber + (_html5Only?' (HTML5-only mode)':' (AS2/Flash 8)');
    // set up default options
    _s.movieURL = 'soundmanager2.swf'.replace('.swf',isDebug);
  };
  ...
  _createMovie = function(smID, smURL) {
    ...
    function _initMsg() {
      //_s._wD('-- SoundManager 2 Basic ' + _s.version + (!_html5Only && _s.useHTML5Audio?(_s.hasHTML5?' + HTML5 audio':', no HTML5 audio support'):'') + (_s.useHighPerformance?', high performance mode, ':', ') + ((_s.useFastPolling?'fast':'normal') + ' polling') + (_s.wmode?', wmode: ' + _s.wmode:'') + (_s.debugFlash?', flash debug mode':'') + (_s.useFlashBlock?', flashBlock mode':'') + ' --', 1);
    }
